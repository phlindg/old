...
class Backtest:
    def __init__(self,csv_dir, symbol_list,start_date, train_date, end_date, initial_capital,
                events, bars_cls, strategy_cls, port_cls, risk_cls, exe_cls, statistics_cls):
        self.events = events
    ...
    def initialize(self):
        ...
        self.strat = self.strategy_cls(self.events, self.bars)
        self.port = self.port_cls(self.events, self.bars, self.initial_capital)
        self.risk = self.risk_cls(self.events, self.port)
    ...
    def run(self):
        ...
        stats = self.stats.output_summary_stats()
...
def multiple_runs(csv_dir, symbol_list,start_date, train_date, end_date, initial_capital,
                events,bars_cls, strategy_cls, port_cls, risk_cls, exe_cls, statistics_cls):
